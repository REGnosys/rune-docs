...
In Rune DSL, all such components are represented using a single unified function construct.
...
Just like spreadsheets let you create formulas and reuse them, Rune DSL lets you build complex processes by combining smaller, reusable function components.
...
Function specifications define the processes used in a Rune DSL domain model. They describe:
...
### Syntax
A function in Rune DSL includes:
 - A name
...
## Function definition
In Rune DSL, you can do more than just declare a function’s inputs and output – you can also define how the output is built.
However, because Rune DSL has limited language features, not all logic can always be fully expressed. This leads to two types of functions:
...
## Function call 
A function call in Rune DSL lets you **execute a function** and use the **output it returns**.
