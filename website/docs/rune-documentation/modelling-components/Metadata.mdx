...
You can attach descriptions to almost any component, including:
- Data types
- Attributes
- Conditions
- Functions
...
- Namespaces
[^1]
[^1]: Should this list of component match our components? Do we need to add any to this list (eg metadata, expressions, data validation, mapping)? Or do we need to remove some (eg attributes, conditions, inputs/outputs, business logic). Or do we simply need to rename some so that they match?
...
Rune DSL includes some built in annotations, but you can define your own as needed. 
...
How to work with metadata that is the output of an expression:
```haskell
func MyFunc:
    inputs:
        myInput string (1..*)
...
The syntax to define a body, corpus and segment:
```haskell
...
**Corpuses** represent document sets such as directives, laws, regulatory texts, technical standards, or best practice guidance. 
...
[^2]
[^2]: Link to deep path operator - Expressions component
...
In this example, marking `ContractualProduct` as a template also makes `EconomicTerms` (and all types inside it) eligible for the template:
```haskell
