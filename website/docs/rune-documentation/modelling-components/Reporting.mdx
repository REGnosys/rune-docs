...
4. [Function overriding](#4-report-rules)


Rune DSL supports financial institutions’ regulatory reporting requirements by allowing reporting rules to be written as **functional components** in the same language used to model the business domain – a **domain model**. Code generators can then turn these rules into executable code that all firms can use consistently.
[^1]

[^1]: Can we add line breaks before each of the headers?
...
`regulatoryReference` links the rule to the exact regulatory text that justifies it and uses the same syntax as `docReference`. In the document reference section of the [Metadata component](https://rune-docs.netlify.app/docs/rune-documentation/Rune%20DSL%20component%20-%20Metadata). 
...
reporting rule EuroEmissionStandard from ReportableEvent:
  ...
  then Euro2Standard
```
The expressions may use any type of [Expression component](https://rune-docs.netlify.app/docs/rune-documentation/Rune%20DSL%20component%20-%20Expressions)  available in Rune DSL, from simple path expressions or constants to more complex conditional statements:
...
### Chaining instructions with `then`
...
### Repeat instruction (repeatable fields)
