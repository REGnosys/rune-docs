...
Mapping in Rune DSL defines **how external documents** (such as FpML or ISDACreate) are **converted into Rune documents**.
This is done using **synonyms**, which are annotations placed on:
- Attributes of data types
- Values of enum types
All synonyms across the model work together to map the structure of the input document into the final Rune DSL document. These mappings can be used to generate an **Ingestion Environment** – a Java library that reads an input document and produces the corresponding Rune DSL document.


## Basic mapping
Basic mappings describe how a value in the input document maps directly to a value in the Rune DSL document.
...
**Inline** (next to the attribute) `[synonym <SynonymSource> <SynonymBody>]`
...
**Externally** (defined inside a separate synonym source block) External synonyms are defined inside a synonym source block:

Example: This removes all existing synonyms on fuel and adds a new one.
```
...
### 1. Hints
Hints allow the algorithm to **skip a layer** in the Rune DSL structure without consuming the input attribute.
Example:
```
type EngineSpecification:
...
### 2. Merging inputs
If a Rune DSL attribute has multiple cardinality, multiple input elements may map to it.
**Without merging**
...
### 5. Mapper
If Rune DSL’s mapping syntax cannot express the required transformation, you can call a **mapper**.
```
fuelType string (0..1)
